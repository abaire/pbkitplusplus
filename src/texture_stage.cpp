...
void TextureStage::Commit(uint32_t memory_dma_offset, uint32_t palette_dma_offset) const {
  ...
  PBKPP_ASSERT(format_.xbox_bpp &&
               "No texture format specified. This will cause an invalid pgraph state exception and a crash.");
  ...
  if (format_.xbox_format == NV097_SET_TEXTURE_FORMAT_COLOR_SZ_I8_A8R8G8B8) {
    PBKPP_ASSERT(palette_length_ <= 3 && "Invalid attempt to use paletted format without setting palette.");
...
int TextureStage::SetTexture(const SDL_Surface *surface, uint8_t *memory_base) const {
  ...
  if (format_.require_conversion) {
    ...
    if (format_.xbox_swizzled) {
      PBKPP_ASSERT(converted && "Swizzle source buffer not populated");
...
int TextureStage::SetVolumetricTexture(const SDL_Surface **layers, uint32_t depth, uint8_t *memory_base) const {
  PBKPP_ASSERT((!format_.xbox_linear) && "Volumetric textures using linear formats are not supported by XBOX.");
  ...
  for (auto i = 0; i < depth; ++i) {
    ...
    if (!new_surfaces[i]) {
      PBKPP_ASSERT(!"Failed to convert surface format.");
      // This will leak memory, but the PBKPP_ASSERT will halt execution anyway.
      return 4;
    }
  }
  ...
  for (auto i = 0; i < depth; ++i) {
    ...
    PBKPP_ASSERT(s->w == width && "Volumetric surface layers must have identical dimensions");
    PBKPP_ASSERT(s->h == height && "Volumetric surface layers must have identical dimensions");
    PBKPP_ASSERT(s->pitch == pitch && "Volumetric surface layers must have identical dimensions");
    PBKPP_ASSERT(s->format->BytesPerPixel == bytes_per_pixel &&
                 "Volumetric surface layers must have identical dimensions");
...
int TextureStage::SetPaletteSize(uint32_t length) {
  switch (length) {
    ...
    case 32:
      ...
      break;
    default:
      PBKPP_ASSERT(!"Invalid palette length. Must be 32|64|128|256");
      return 1;
  }
  return 0;
}
