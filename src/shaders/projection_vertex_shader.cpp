...
int ProjectionVertexShader::OnLoadConstants() {
  ...
  upload_matrix(model_matrix_);                // 96
  upload_matrix(view_matrix_);                 // 100
  upload_matrix(projection_viewport_matrix_);  // 104

  upload_vector(camera_position_);  // 108

  // #zero_constant
  float constants_0[4] = {0, 0, 0, 0};
  upload_vector(constants_0);  // 109

  // #basic_constants
  float constants_basic[4] = {-1.f, -0.5f, 0.5f, 1.f};
  upload_vector(constants_basic);  // 110
...
void ProjectionVertexShader::CalculateViewportMatrix() {
  if (use_d3d_style_viewport_) {
    ...
    PBKPP_ASSERT(z_min_ == 0.0f && "Viewport z-range only implemented for 0..1");
