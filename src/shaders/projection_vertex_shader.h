...
class ProjectionVertexShader : public VertexShaderProgram {
 public:
  ProjectionVertexShader(uint32_t framebuffer_width, uint32_t framebuffer_height, float z_min = 0,
                         float z_max = 0x7FFF);
  ...
  [[nodiscard]] inline float GetZMin() const { return z_min_; }
  [[nodiscard]] inline float GetZMax() const { return z_max_; }
