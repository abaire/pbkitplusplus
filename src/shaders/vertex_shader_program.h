#ifndef PBKITPLUSPLUS_VERTEX_SHADER_PROGRAM_H
#define PBKITPLUSPLUS_VERTEX_SHADER_PROGRAM_H
...
class VertexShaderProgram {
 ...
 public:
  ...
  //! Sets the vertex shader machine code.
  //! Note: It is the caller's responsibility to ensure that the shader array remains valid for the lifetime of this
  //! instance.
  VertexShaderProgram(const uint32_t *shader, uint32_t shader_size) : VertexShaderProgram() {
    SetShader(shader, shader_size);
  };
  ...
  //! Note: It is the caller's responsibility to ensure that the shader array remains valid for the lifetime of this
  //! instance.
  void SetShader(const uint32_t *shader, uint32_t shader_size) {
    shader_ = shader;
    shader_size_ = shader_size;
  }
 ...
 protected:
  ...
  //! Called to allow subclasses to set shader constants using the SetBaseUniform* methods.
  //!
  //! \return int - The next available shader constant index after loading is completed.
  virtual int OnLoadConstants() { return 0; };
 ...
 protected:
  ...
  const uint32_t *shader_{nullptr};
  uint32_t shader_size_{0};
...
#endif  // PBKITPLUSPLUS_VERTEX_SHADER_PROGRAM_H
